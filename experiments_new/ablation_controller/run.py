...
multi-chain FMP configuration (ladder step iv); only the regime controller differs.
    ...
    CTRL_HMM            3-state HMM, soft filter, Baum-Welch on A (hand-set emissions)
    CTRL_HMM_VITERBI    same with hard Viterbi decoding
    CTRL_HMM_NOBW       hand-set A, no online learning
    CTRL_HMM_LEARNEMIS  Baum-Welch also updates emission means / variances
    CTRL_FIXED          always EXPLOIT weights (TRAPPED only via the rejection counter)
    CTRL_RANDOM         uniformly random EXPLOIT / EXPLORE each step
    CTRL_RULE           threshold rule: EXPLOIT if mean(O_window) <= 0.05 else EXPLORE
...
METHODS = ["CTRL_HMM", "CTRL_HMM_VITERBI", "CTRL_HMM_NOBW", "CTRL_HMM_LEARNEMIS",
           "CTRL_FIXED", "CTRL_RANDOM", "CTRL_RULE"]
