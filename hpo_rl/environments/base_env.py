...
class BaseHPOEnv(gym.Env, ABC):
    ...
    def _assemble_config(self, chosen_options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        В текущей реализации просто возвращает входные данные без изменений.
        Это точка расширения для будущих версий, где может потребоваться
        дополнительная обработка конфигурации (например, добавление метаданных,
        валидация, нормализация и т.д.).
