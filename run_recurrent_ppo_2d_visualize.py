"""
Отдельный скрипт для экспериментов с RecurrentPPO.
Использует оценку по финальной конфигурации эпизода, а не по лучшей промежуточной точке.
Это критично для правильной оценки рекуррентных алгоритмов.
"""
import yaml
import argparse
import os
...
def collect_trajectory(agent, env, num_episodes: int = 1, eval_seed: Optional[int] = None) -> Tuple[List[Tuple[float, float, float]], Tuple[float, float, float]]:
    """
    Собирает траекторию поиска: (x0, x1, metric)
    Собирает все точки, которые посещает агент (current_config), а не только лучшие
    
    ВАЖНО: Возвращает финальную конфигурацию эпизода как результат алгоритма.
    Это критично для правильной оценки, особенно для рекуррентных алгоритмов (RecurrentPPO),
    которые должны учиться последовательности действий, ведущей к хорошему финальному результату,
    а не просто запоминать лучшую промежуточную точку.
    
    Args:
        eval_seed: Seed для инициализации стартовой точки (None = случайный)
    
    Возвращает: (trajectory, final_point) где final_point - финальная конфигурация эпизода
    """
    trajectory = []
    step_sizes_used = []  # Для статистики
    final_point = None  # Финальная конфигурация эпизода
    vec_env = agent.get_env()
    
    for episode in range(num_episodes):
        # Устанавливаем seed для стартовой точки если указан
        if eval_seed is not None:
            vec_env.seed(eval_seed + episode)  # Разный seed для каждого эпизода
        
        # VecEnv.reset() возвращает только наблюдения, не кортеж
        obs = vec_env.reset()
        done = False
        step_count = 0
        _states = None  # Инициализируем состояния для RecurrentPPO
        
        # Получаем начальную точку из первого шага
        # (так как reset() не возвращает info в VecEnv)
        # ВАЖНО: Для RecurrentPPO нужно передавать states обратно в predict
        action, _states = agent.predict(obs, state=_states, deterministic=True, episode_start=np.array([True]))
        ...
        while not done:
            # КРИТИЧНО: Передаем _states обратно в predict для RecurrentPPO
            # episode_start=False, так как это не начало эпизода (мы уже сделали первый шаг)
            action, _states = agent.predict(obs, state=_states, deterministic=True, episode_start=np.array([False]))
            obs, reward, dones, infos = vec_env.step(action)
            done = dones[0] if isinstance(dones, (list, np.ndarray)) else dones
            
            # Если эпизод завершился, сбрасываем состояния для следующего эпизода
            # Это важно для LSTM, чтобы он сбросил память между эпизодами
            if done:
                _states = None
            ...
            # Сохраняем финальную конфигурацию (последняя перед завершением)
            if done:
                if len(current_config) >= 2:
                    x0 = current_config.get("x0", 0.0)
                    x1 = current_config.get("x1", 0.0)
            ...
            if step_count > 1000:  # Защита от бесконечного цикла
                # Если вышли по защите, используем последнюю точку как финальную
                if final_point is None and len(trajectory) > 0:
                    final_point = trajectory[-1]
                break
        
        # Если эпизод завершился, но final_point не установлен, используем последнюю точку
        if final_point is None and len(trajectory) > 0:
            final_point = trajectory[-1]
...
def visualize_2d_trajectory(backend, trajectory: List[Tuple[float, float, float]], 
                           final_point: Tuple[float, float, float] = None,
    ...
    # Отмечаем финальную точку как результат алгоритма (а не лучшую промежуточную)
    final_idx = len(trajectory) - 1  # Индекс для 3D визуализации (fallback)
    if final_point is not None:
        final_x0, final_x1, final_metric = final_point
        # Конвертируем reward в значение функции
        if backend.maximize:
            final_value = final_metric
        else:
            final_value = -final_metric
        ax1.scatter(final_x0, final_x1, c='yellow', s=150, 
                    marker='X', label='Финальная точка (результат)', zorder=5, edgecolors='black', linewidths=2)
    else:
        # Fallback: используем последнюю точку траектории
        ax1.scatter(x0_vals[final_idx], x1_vals[final_idx], c='yellow', s=150, 
                    marker='X', label='Финальная точка (результат)', zorder=5, edgecolors='black', linewidths=2)
    ...
    if final_point is not None:
        ...
        if backend.maximize:
            ...
            final_value = -final_metric
        ax2.scatter(final_x0, final_x1, final_value, 
                    c='yellow', s=150, marker='X', label='Финальная точка (результат)', 
                    edgecolors='black', linewidths=2)
    else:
        ax2.scatter(x0_vals[final_idx], x1_vals[final_idx], metrics[final_idx], 
                    c='yellow', s=150, marker='X', label='Финальная точка (результат)', 
                    edgecolors='black', linewidths=2)
...
def run_experiment_with_visualization(config: Dict[str, Any], pretrained_model_path: str = None, 
    ...
    if transfer_learning and pretrained_model_path:
        ...
        # Принудительно устанавливаем use_history=False для RecurrentPPO
        # так как модель была обучена без истории
        env_cfg['params']['use_history'] = False
        print(f"✓ Принудительно установлено use_history=False для RecurrentPPO")
    ...
    trajectory, final_point = collect_trajectory(agent, env, num_episodes=1, eval_seed=eval_seed)
    print(f"Собрано {len(trajectory)} точек траектории")
    
    if len(trajectory) > 0:
        # Используем финальную точку как результат алгоритма (а не лучшую промежуточную)
        if final_point is not None:
            final_x0, final_x1, final_metric_reward = final_point
            # Конвертируем reward в значение функции
            if backend.maximize:
                final_metric = final_metric_reward
            else:
                final_metric = -final_metric_reward
        else:
            # Fallback: используем последнюю точку
            final_metric_reward = trajectory[-1][2]
            if backend.maximize:
                final_metric = final_metric_reward
            else:
                final_metric = -final_metric_reward
        
        print(f"Финальная метрика (значение функции в конце эпизода): {final_metric:.6f}")
        print("  ⚠️  ВАЖНО: Оценка ведется по финальной конфигурации, а не по лучшей промежуточной!")
        print("  Это правильно для оценки алгоритма, особенно для рекуррентных моделей.")
        if hasattr(backend, 'global_optimum_value'):
            print(f"Глобальный оптимум: {backend.global_optimum_value:.6f}")
            print(f"Отклонение от оптимума: {abs(final_metric - backend.global_optimum_value):.6f}")
        
        # Для справки показываем также лучшую промежуточную точку
        rewards = [t[2] for t in trajectory]
        if backend.maximize:
            metrics = rewards
        ...
        print(f"Лучшая промежуточная метрика (для справки, не используется для оценки): {best_intermediate:.6f}")
        ...
        visualize_2d_trajectory(backend, trajectory, final_point=final_point, save_path=save_path)
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Визуализация 2D оптимизации с RecurrentPPO (оценка по финальной точке)"
    )
    parser.add_argument("--config", type=str, default="configs/function_2d_recurrent_ppo.yaml",
    ...
    parser.add_argument("--agent", type=str, default="RecurrentPPO",
                       help="Имя алгоритма для использования (по умолчанию RecurrentPPO)")
