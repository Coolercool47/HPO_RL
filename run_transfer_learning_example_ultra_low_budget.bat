@echo off
REM Пример скрипта для демонстрации transfer learning с ультрамалыми бюджетами (Windows)
REM Использование: run_transfer_learning_example_ultra_low_budget.bat [ALGORITHM]
REM Пример: run_transfer_learning_example_ultra_low_budget.bat PPO
REM Пример: run_transfer_learning_example_ultra_low_budget.bat PPO_PER_CYCLE
REM Пример: run_transfer_learning_example_ultra_low_budget.bat TD3
REM Пример: run_transfer_learning_example_ultra_low_budget.bat SAC
REM Пример: run_transfer_learning_example_ultra_low_budget.bat RecurrentPPO
...
REM Определяем конфиг в зависимости от алгоритма (с ультрамалыми бюджетами)
if "%AGENT%"=="TD3" (
    set CONFIG=configs/function_2d_td3_ultra_low_budget.yaml
    set MODEL_FILE=model_2d_td3_ultra_low_budget.zip
) else if "%AGENT%"=="SAC" (
    set CONFIG=configs/function_2d_sac_ultra_low_budget.yaml
    set MODEL_FILE=model_2d_sac_ultra_low_budget.zip
) else if "%AGENT%"=="PPO_PER_CYCLE" (
    set CONFIG=configs/function_2d_ppo_per_cycle_ultra_low_budget.yaml
    set MODEL_FILE=model_2d_ppo_per_cycle_ultra_low_budget.zip
    set AGENT=PPO
    ...
    set CONFIG=configs/function_2d_test_ultra_low_budget.yaml
    set MODEL_FILE=model_2d_ultra_low_budget.zip
)

echo === Шаг 1: Обучение модели на Rastrigin (ультрамалый бюджет) ===
...
python run_experiment_2d_visualize.py --config configs/function_2d_sphere.yaml --pretrained-model logs/%AGENT%/%MODEL_FILE% --transfer-learning --fine-tune-steps 600 --exploration-boost 2.5 --agent %AGENT% --eval-seed 42

echo.
echo === Шаг 8: Дообучение на Rosenbrock (fine-tuning) ===
python run_experiment_2d_visualize.py --config configs/function_2d_rosenbrock.yaml --pretrained-model logs/%AGENT%/%MODEL_FILE% --transfer-learning --fine-tune-steps 600 --exploration-boost 2.5 --agent %AGENT% --eval-seed 42

echo.
echo === Шаг 9: Дообучение на Ackley (fine-tuning) ===
python run_experiment_2d_visualize.py --config configs/function_2d_ackley.yaml --pretrained-model logs/%AGENT%/%MODEL_FILE% --transfer-learning --fine-tune-steps 600 --exploration-boost 2.5 --agent %AGENT% --eval-seed 42

echo.
echo === Готово! Проверьте графики в logs/%AGENT%/ ===
echo Графики сохранены с суффиксом _seedN для разных стартовых точек
echo ВНИМАНИЕ: Использовались ультрамалые бюджеты для обучения
pause
