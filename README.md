...
- **Tree-Structured Parzen Estimator** - ссылки на статьи
- **Hyperband**
...
Логгирование в .csv и .tex в виде таблицы исследованных точек # можно воткнуть пример
...
Эксперимент с настраивоемой конфигурацией по обучением алгоритма hpo_rl оптимизировать гиперпараметры заданной функции.
```bash
...
Эксперимент с настраивоемой конфигурацией по обучением алгоритма hpo_rl оптимизировать гиперпараметры простой модели сверточной нейронной сети (Convolutional Neural Network, CNN) обучаемой на датасете CIFAR100.
```bash
